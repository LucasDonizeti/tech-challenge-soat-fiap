...
## Contexto
O projeto Tech Challenge SOAT FIAP exige alta qualidade de código e manutenibilidade como parte dos critérios de avaliação. Atualmente, o projeto possui cobertura de testes de 80% (ADR-012), mas falta uma ferramenta sistemática para análise estática de código que identifique:
...
A implementação de análise estática contínua é essencial para manter a qualidade do código ao longo do desenvolvimento e garantir que novos commits sigam os padrões estabelecidos.
...
- **SonarQube Server:** Instalação local via Docker Compose para ambiente de desenvolvimento
- **SonarScanner:** Integração com Maven para análise automatizada no pipeline de CI/CD
...
- **Integração com PR:** Análise automática em Pull Requests antes do merge
...
- **Integração nativa:** Excelente integração com Maven, Jenkins/GitHub Actions e IDEs
...
- **Alinhamento com ADR-012:** Complementa a exigência de cobertura de testes com análise de qualidade
...
### Benefícios
- **Qualidade assegurada:** Detecção precoce de problemas de código
...
- **Integração com CI/CD:** Análise automática em cada commit/PR
...
- **Performance:** Análise pode adicionar tempo ao pipeline de build
